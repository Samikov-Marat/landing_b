...
class TextCsv
{
    ...
    {
        return static::PREFIX_NEWS_ARTICLE . $newsArticle->header;
    }
    ...
    public function start($site)
    {
        ...
        foreach ($site->newsArticles as $newsArticle) {
            $newsArticleHeader = [static::getNewsArticlePrefix($newsArticle),];
            foreach ($site->languages as $language) {
                $newsArticleHeader[] = '';
            }
            ...
            foreach (['header', 'note', 'text', 'publication_date_text'] as $attribute) {
                $line = $this->getNewsArticleAttribute($site, $newsArticle, $attribute);
                $this->put($line);
            }
        }
    ...
    public function getNewsArticleAttribute($site, $newsArticle, $attribute)
    {
        $superId = implode('.', ['news_article', $newsArticle->id, $attribute]);
        $line = [$superId,];
        foreach ($site->languages as $language) {
            if ($newsArticle->language_id == $language->id) {
                $line[] = $newsArticle->getAttribute($attribute);
            } else {
                $line[] = '';
            }
        }
